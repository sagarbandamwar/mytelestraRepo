...
class AdapterRow constructor(mContext: Context, mArrayListString: ArrayList<Blog>) :
    ...
    override fun onBindViewHolder(holder: Holder, position: Int) {
        ...
        var mTitle: String? =
            titles.title      // we are retriving title from the array list which received from view model
        holder.mTitle.text = mTitle
        holder.mSubTitle.text =
            titles.description      // we are retriving description from the array list which received from view model
        val mImageUrl: String? =
            titles.imageHref       // we are retriving image LINK from the array list which received from view model

        //picasso.setIndicatorsEnabled(true)
        mImageUrl.let {
            Picasso.get()
                .load(mImageUrl)                          // we are loading images with the help of picasso
